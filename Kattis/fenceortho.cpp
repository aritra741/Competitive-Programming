...
int main()
{
    ...
    while(scanf("%d", &n)==1)
    {
        ...
        for( ;i<n;i++ )
        {
            ...
            double a= cross( ret[(i+1)%n]-ret[i], ret[j]-ret[i] )/( ret[(i+1)%n]-ret[i] ).val(); /// getting the height
            double b= (dot( ret[(i+1)%n]-ret[i], ret[i2]-ret[i] )-dot( ret[(i+1)%n]-ret[i], ret[j2]-ret[i] ))/( ret[(i+1)%n]-ret[i] ).val(); /// getting the width
            double peri= (a+b)*2.00;
