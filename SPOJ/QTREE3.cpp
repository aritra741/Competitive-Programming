...
vector <int> adj[N];
int baseArray[N], ptr, val[N];
int chainNo, chainInd[N], chainHead[N], posInBase[N];
int depth[N], pa[LN][N], subsize[N], nodepos[N];
int st[N*6], qt[N*6];
...
//    cout<<nodepos[s]<<" "<<nodepos[e]<<" "<<st[cur]<<" "<<st[c1]<<" "<<st[c2]<<endl;
}
...
//    cout<<nodepos[s]<<" "<<nodepos[e]<<" "<<st[cur]<<" "<<st[c1]<<" "<<st[c2]<<" ";
//    cout<<"ans: ";
//    if( qt[cur]==inf )
//        cout<<"-1"<<endl;
//    else
//        cout<<nodepos[ qt[cur] ]<<endl;
}
...
int query_up(int u, int v)
{
    ...
    while(1)
    {
        ...
        if(uchain == vchain)
        {
            ...
            ans=min(ans,qt[1]); // Update answer
            break;
        }
...
void query(int u, int v)
{
    ...
    ans= min(ans,temp); // take the minimum of both paths
    ans= (ans==inf)?-1:nodepos[ans]+1;
