...
//    cin.tie(0);
    ...
    for( int i=s.size();i<save[0].size();i++ )
    {
        if( save[0][i]+save[1][i]+save[2][i]>ans )
            ans= save[0][i]+save[1][i]+save[2][i], v.clear(), v.push_back(i-s.size()+1);
        else if( save[0][i]+save[1][i]+save[2][i]==ans )
